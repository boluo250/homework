...
- 搜索：普通实时问题和深度研究子任务共用同一个 `SearchService`，底层默认走 Serper；未配置 `SERPER_API_KEY` 时返回明确的配置提示。
...
| `SERPER_API_KEY` | Serper 实时网页搜索 |
| `SERPER_SEARCH_GL`、`SERPER_SEARCH_HL` | 可选，配置 Serper 默认地域/语言参数 |
...
5. 设置 secrets（如需固定地域/语言，也可额外设置 `SERPER_SEARCH_GL` / `SERPER_SEARCH_HL`）：
