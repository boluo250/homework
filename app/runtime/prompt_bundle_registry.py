...
def build_router_prompt_bundle(
    *,
    ...
    context_parts = [
        ...
        "- For update_task, title is the task to locate now; new_title is the renamed title after the update. Never overwrite title with the new title.",
        "- For update_task, only modify fields the user explicitly asked to change. Leave all other fields unset so they remain unchanged.",
        "- For update_task, if the user only says rename / 改标题 / 改名字, call update_task with new_title only and do not rewrite unrelated fields.",
